...
sudo apt-get update -y > /dev/null 2>&1 || error "Failed to update package lists."

info "Installing essential tools (curl, git)..."
sudo apt-get install -y curl git > /dev/null 2>&1 || error "Failed to install curl or git."
...
if command_exists node && command_exists npm; then
  ...
  curl -fsSL https://deb.nodesource.com/setup_18.x | sudo bash - > /dev/null 2>&1
...
if command_exists pm2; then
  sudo pm2 startup systemd -u root --hp /root > /dev/null 2>&1 || warn "PM2 startup command might have had issues. You may need to run 'sudo pm2 startup' manually and follow instructions."
...
if command_exists nginx; then
  ...
  sudo apt-get install -y nginx > /dev/null 2>&1 || error "Failed to install Nginx."
...
if command_exists certbot; then
  ...
  sudo apt-get install -y certbot python3-certbot-nginx > /dev/null 2>&1 || error "Failed to install Certbot or its Nginx plugin."
...
sudo curl -fsSL "${PB_MANAGER_SCRIPT_URL}" -o "${PB_MANAGER_INSTALL_DIR}/pb-manager.js" > /dev/null 2>&1 || error "Failed to download pb-manager.js."
...
sudo npm install commander inquirer@8.2.4 fs-extra axios chalk@4.1.2 unzipper shelljs blessed blessed-contrib cli-table3 pretty-bytes@5.6.0 > /dev/null 2>&1 || error "Failed to install pb-manager dependencies."
